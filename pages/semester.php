...
if (!empty($_POST)) {
    ...
    if (isset($_POST['startYear'])) {
        $s_startYear = $_POST['startYear'];
    }

    if (isset($_POST['endYear'])) {
        $s_endYear = $_POST['endYear'];
    }
    ...
    $s_startYear = str_replace('\'', '\\\'', $s_startYear);
    $s_endYear = str_replace('\'', '\\\'', $s_endYear);
    $s_id = str_replace('\'', '\\\'', $s_id);

    if ($s_id != '') {
        //update
        $sql = "update semester set type = '$s_type', startYear = '$s_startYear', endYear = '$s_endYear' where id = " . $s_id;
    } else {
        //insert
        $sql = "insert into semester(type, startYear, endYear) value ('$s_type', '$s_startYear', '$s_endYear')";
    }
...
<body id="page-top">
    ...
    <div id="wrapper">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h3 class="h3 mb-0 text-gray-800">BẢNG QUẢN LÝ KỲ HỌC</h3>
                    </div>
                    <div class="row">
                        <div class="card">
                            <div class="card-body">
                                    ...
                                    <table class="table table-bordered table-responsive-md table-striped text-center">
                                        ...
                                        <tbody>
                                            ...
                                            if (isset($_GET['s']) && $_GET['s'] != '') {
                                                $sql = 'select id,type,startYear,endYear from semester where type like "%' . $_GET['s'] . '%"';
                                            } else {
                                                $sql = 'select id,type,startYear,endYear from semester';
                                            }
                                            $semesterList = executeResult($sql);
                                            $index = 1;
                                            ?>

                                            <?php foreach ($semesterList as $std) : ?>
                                                <form>
                                                    ...
                                                    <script>
                                                        $(document).ready(function() {
                                                            $('#edit<?= $std['id'] ?>').click(function(e) {
                                                                ...
                                                                e.preventDefault();
                                                                $.ajax({
                                                                    url: '../controllers/edit_semester.php',
                                                                    data: {
                                                                        'id': <?= $std['id'] ?>
                                                                    },
                                                                    type: 'POST',    
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">Thêm kì học</h5>
        ...
        <script type="text/javascript">
            function deleteS(id) {
                option = confirm('Bạn có muốn xoá kì học này không')
                if (!option) {
                    return;
                }

                console.log(id)
                $.post('delete_semester.php', {
                    'id': id
                }, function(data) {
                    alert(data)
                    location.reload()
